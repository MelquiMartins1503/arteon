...
} from "@/utils/schemas/passwordRecovery";
...
const Page: NextPage = () => {
  ...
  const handleResend = () => {
    startTransition(async () => {
      try {
        await apiClient.post("/forgot-password", { email: submittedEmail });
  ...
  return (
    <Box className="p-4 w-full h-full">
      <Box className="flex-1 h-full rounded-2xl from-brand-200 bg-linear-to-b to-brand-400">
        a
      </Box>
                  ...
                  <>
                    <Form.Header.Description>
                      ...
                      <Typography
                        size="xl"
                        weight="medium"
                        className="text-brand-975 dark:text-brand-50"
                      >
                        {maskEmail(submittedEmail)}
                      </Typography>{" "}
                      corresponder a um endereço de e-mail que temos em nossos
                      registros, enviamos um e-mail con instruções adicionais
